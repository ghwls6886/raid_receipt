...
- [x] 개인 보스 추적 (`boss-tracker`) (2026-08-07)
      ↳ 월별 이력 화면(`BossCalendarView` + `BossHistoryPage` + `getCharBossEntriesByMonth`)은
        2026-08-07 에 마저 옮겼다. `/boss-tracker/history`
      ↳ **1트/2트 그룹핑을 처음 이식할 때 빠뜨렸다** — MH `groupTimers()` 를 안 옮기고 평면
        목록으로 렌더했다. 2026-08-07 복원. 같은 보스+캐릭터를 묶고 헤더에 "N/M트 진행 중"
      ↳ 보스 색을 **계열이 아니라 색상환으로** 다시 배정했다. 자쿰/카오스 자쿰을 같은 amber
        농도차로 두니 나란히 놓였을 때 둘 다 "주황"으로 읽혀 구분이 안 됐다. 이름에 "카오스"가
        이미 있으니 색까지 같은 계열일 필요가 없다. 테두리 채도도 200 → 500
...
**2단계 완료 (잔여 없음, 2026-08-07).** 길드 없는 사용자가 쓸 화면이
캐릭터 · 숙제 · 보스타이머 · 입장 기록 4개 생겼다.

### 3단계 — 랜딩·도메인

- [x] 랜딩 3분할 + 허브 신규 — §5 (2026-08-07)
      ↳ 두 번째 랜딩은 `/party` 가 아니라 **`/helper`**. 구인은 4단계라 없고, 2단계에서 만든 건
        개인 도구다. maple_helper 는 "헬퍼"라는 한 제품이고 구인은 그 안의 기능이다
      ↳ 헬퍼 랜딩은 **지금 있는 기능만** 광고한다. MH 랜딩의 버프콜·올인원 대시보드는 뺐다
      ↳ 라이트 고정 17줄을 `hooks/useForceLightTheme` 로 추출. 허브도 같은 규칙 (§5 미결 해소)
- [x] 제품 스위처 (TopNav) (2026-08-07)
      ↳ **버그를 하나 고쳤다.** 1단계의 `navItems = hasGuild ? SETTLEMENT_NAV : HELPER_NAV` 는
        길드가 있으면 헬퍼 내비를 영영 숨겼다 — 길드원은 캐릭터·숙제·보스타이머로 갈 길이 없었다.
        이제 현재 제품을 **경로로 판정**한다
      ↳ 길드 스위처는 정산 제품에서만. 헬퍼는 길드와 무관하다
- [x] 문맥 넛지 2곳 (2026-08-07) — **§6 이 지정한 것과 다른 두 개다**
      ↳ §6 의 원안("구인 모집 완료 → 공대 만들고 정산", "공대 인원 부족 → 구인 글 올리기")은
        **둘 다 구인이 전제**라 4단계 전에는 만들 수 없다. 지금 가능한 정산 ↔ 헬퍼로 대체했다
      ↳ 헬퍼 → 정산: 캐릭터를 등록했고 길드가 없을 때 (`/characters`).
        캐릭터 등록 = 제품을 실제로 쓰기 시작한 시점이라 그때 꺼낸다. 빈 화면에서 들이밀면 광고다
      ↳ 정산 → 헬퍼: 대시보드 하단. 헬퍼 데이터로 "이미 쓰는 사람"을 거르지 **않는다** —
        그러려면 정산이 `features/helper` 를 import 해야 해 §4.1 원칙 3 위반이다.
        대신 닫으면 다시 안 뜬다
      ↳ `components/common/CrossProductNudge` 공용 컴포넌트. 4단계의 구인 넛지도 이걸 쓴다
...
- [x] `0015_helper_recruit.sql` ← **번호 주의.** `0014` 는 `0014_admin_master_write.sql` 이 가져갔다
- [x] 파티 구인 · 지원 · 파티 채팅(realtime) · 매너 평가
- [ ] realtime 동시접속 200(무료 한도)이 여기서 걸린다 — §8 *(아직 사용자 없어 미검증)*
- [x] **버프콜** — 2단계에서 옮겨왔다 (2026-08-07)

> **4단계 완료 (2026-08-07).** 커밋 `2ad2391`(목록) → `5a32862`(파티방) →
> `c68c51f`(매너 평가) → `d51b1a0`(심콜). **이것으로 maple_helper 이식이 전부 끝났다.**
>
> 이식하지 않은 것과 이유:
> - `MannerProfileCard` — 원본에서 사용처 0건
> - helper 올인원 대시보드 — RR 이 허브·랜딩으로 대체
> - `RatingsPage` 는 이식하되 "받은 평가 이력"이 아니라 **평가 대기 목록 + 내 매너온도**로 바꿨다.
>   받은 평가는 익명이어야 해서 0015 정책이 `rater_id = auth.uid()` 만 열어 준다
>
> 원본의 버그를 고친 것 (그대로 옮기지 않았다):
> - 평가 화면의 상대 매너온도가 항상 30.0°C 였다. 참가자 스냅샷에 온도가 없다는 이유로
>   기본값을 넣고 있었는데, 온도는 스냅샷이 아니라 볼 때마다 읽을 값이다
> - `RATING_TRIGGER_LABELS` 에 `MEMBER_KICKED` 가 없었다 (0015 의 check 는 3종)
> - 오디오 설정의 볼륨·속도 슬라이더가 아무 데도 연결돼 있지 않았다
> - 프리셋 id 가 전역 카운터라 새로고침 후 기존 프리셋과 부딪혔다
...
- **`workers/buff-timer.worker.ts` + `workers/types.ts`**
  ...
  → **처리 완료.** 별칭을 시험하지 않고 처음부터 워커를 `features/recruit/buff/` 안에 두고
  `new URL('./buff-timer.worker.ts', import.meta.url)` 로 가리켰다. 프로덕션 빌드에
  `dist/assets/buff-timer.worker-*.js` 청크가 나오는 것까지 확인했다.
...
- [ ] 허브의 다크모드 정책 (§5)
- (2단계 잔여 없음 — 보스 월별 이력까지 2026-08-07 에 완료)
